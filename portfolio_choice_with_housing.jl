...
@with_kw struct Model_Parameters
    ...
    R_D::Float64 = compound(0.04, 5)
    R_F::Float64 = compound(0.02, 5)
    R_S::Float64 = 0.1
    μ::Float64 = compound(log(R_S + 1.0) - σ_η/2, 5)
    ...
    δ::Float64 = 0.01
    λ::Float64 = 0.08
    b::Float64 = compound(0.01, 5)

    # Utility function parameters
    θ::Float64 = 0.1
    γ::Float64 = 5.0
    β::Float64 = compound(0.96,5)
    ...
    P_bar::Float64 = 30500.0 * 140.3/41.8
    np::Int64 = nη
...
# Takes states X, H_t, η, Inv Move, IFC, 
# choices C, H_prime,  D, α, FC 
# and random variables ω_prime, r_prime, η_prime 
# and outputs the sum of stocks and bonds
...
function Solve_Worker_Problem(para::Model_Parameters, sols::Solutions)
    ...
    for j in T:-1:TR  # Backward induction
        ...
        # Generate interpolation functions for each possible cash on hand tomorrow 
                                                ...
                                                for η_prime_index in 1:nη
                                                    ...
                                                    for ω_prime_index in 1:nω
                                                        ...
                                                        for ι_prime_index in 1:nr 
                                                            ...
                                                            val += (1-π) * Prob(η_index, η_prime_index, ω_index, ω_prime_index, ι_index, ι_prime_index) *
                                                                   interp_functions[((H_prime_index - 1) * nη * 2 * 2) + ((η_prime_index - 1) * 2 * 2) + (0 * 2) + FC + 1](X_prime) +
                                                                   π * Prob(η_index, η_prime_index, ω_index, ω_prime_index, ι_index, ι_prime_index) *
                                                                   interp_functions[((H_prime_index - 1) * nη * 2 * 2) + ((η_prime_index - 1) * 2 * 2) + (1 * 2) + FC + 1](X_prime)

                                                            # Update value function
                                                            if val > candidate_max 
                                                                val_func[H_index, X_index, η_index, Inv_Move + 1, IFC + 1] = val
                                                                c_pol_func[H_index, X_index, η_index, Inv_Move + 1, IFC + 1] = c 
                                                                H_pol_func[H_index, X_index, η_index, Inv_Move + 1, IFC + 1] = H_prime
                                                                D_pol_func[H_index, X_index, η_index, Inv_Move + 1, IFC + 1] = D
                                                                FC_pol_func[H_index, X_index, η_index, Inv_Move + 1, IFC + 1] = 0
                                                                α_pol_func[H_index, X_index, η_index, Inv_Move + 1, IFC + 1] = α
                                                            end 
                                                        end 
                                                    end 
                                                end 
                                                        ...
                                                        for ω_prime_index in 1:nω
                                                            ...
                                                            for ι_prime_index in 1:nr 
                                                                ...
                                                                if val > candidate_max 
                                                                    ...
                                                                    H_pol_func[H_index, X_index, η_index, Inv_Move + 1, IFC + 1] = H_prime
                                                                    D_pol_func[H_index, X_index, η_index, Inv_Move + 1, IFC + 1] = D
                                                                    FC_pol_func[H_index, X_index, η_index, Inv_Move + 1, IFC + 1] = FC
                                                                    α_pol_func[H_index, X_index, η_index, Inv_Move + 1, IFC + 1] = α
                                                                end 
                                                            end 
                                                        end
